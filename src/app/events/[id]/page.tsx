  ...
  const handleCancelRegistration = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(error.error || 'Gagal membatalkan pendaftaran');
      }
    } catch (error) {
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden mb-6">
          <div className="bg-gradient-to-r from-green-600 to-emerald-600 px-8 py-6">
            <div className="flex items-start justify-between">
              ...
              <div className="flex items-center gap-2 px-4 py-2 bg-white/20 backdrop-blur-sm rounded-full border border-white/30">
                ...
                <span className="text-white font-semibold">{eventData.registrationCount || 0} peserta</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-8">
            ...
            {/* Contact Person - Always Show (Fallback to Organizer) */}
            <div className="flex items-center gap-3 p-4 bg-blue-50 rounded-xl border border-blue-200 mt-4">
              ...
              <div>
                ...
                {/* Name: Specific or Organizer */}
                <p className="font-semibold text-gray-900">
                  {ev.contactPersonName || organizer?.fullName || 'Panitia Acara'}
                </p>

                {/* Phone: Only if specific phone is set */}
                {ev.contactPersonPhone && (
                  <a href={`https://wa.me/${ev.contactPersonPhone.replace(/[^0-9]/g, '')}`} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:underline flex items-center gap-1 mt-1">
                ...
                {/* Email: Specific or Organizer */}
                {(ev.contactPersonEmail || organizer?.email) && (
                  <a href={`mailto:${ev.contactPersonEmail || organizer?.email}`} className="text-sm text-blue-600 hover:underline flex items-center gap-1 mt-1">
        ...
        {isOrganizer && eventData.registrations && (
          <div className="bg-white rounded-2xl shadow-xl p-8 mb-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold text-gray-900">
                Peserta Terdaftar ({eventData.registrations.filter(r => r.registration.status === 'registered').length})
              </h3>
              <button
              ...
              <div className="space-y-3">
                {eventData.registrations.filter(r => r.registration.status === 'registered').length === 0 ? (
                  ...
                  eventData.registrations
                    .filter(r => r.registration.status === 'registered')
                    .map((item, idx) => (
                      <div key={item.registration.id} className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl border border-gray-200">
                        <div className="w-10 h-10 bg-gradient-to-br from-green-400 to-emerald-500 rounded-full flex items-center justify-center text-white font-bold">
                          {idx + 1}
                        </div>
                        <div className="flex-1">
                          <p className="font-semibold text-gray-900">{item.user?.fullName || 'Unknown'}</p>
                          <p className="text-sm text-gray-500">{item.user?.email}</p>
                        </div>
                        <div className="text-xs text-gray-400">
                          {item.registration.registeredAt ? new Date(item.registration.registeredAt).toLocaleDateString('id-ID') : '-'}
                        </div>
                      </div>
                    ))
                )}
