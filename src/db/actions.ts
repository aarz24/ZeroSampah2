...
// New: get user's total points from Users table
export async function getUserPoints(userId: string) {
  try {
    const [user] = await db.select({ points: Users.points }).from(Users).where(eq(Users.clerkId, userId)).execute();
    if (!user) return 0;
    return (user.points as number) || 0;
  } catch (error) {
...
// New: add points to user's total (increments Users.points)
export async function addPointsToUser(userId: string, pointsToAdd: number) {
  try {
    const [updated] = await db
      .update(Users)
      .set({ points: sql`${Users.points} + ${pointsToAdd}`, updatedAt: new Date() })
...
export async function getWasteCollectionTasks(limit: number = 20) {
  try {
    const tasks = await db
      ...
      .from(Reports)
      // Show reports that are still pending or currently in-progress.
      // This prevents reports from disappearing from the collectors' queue when
      // someone only inspects the report. The UI already handles presentation
      // for 'in_progress' items (shows who is handling it).
      .where(sql`${Reports.status} IN ('pending','in_progress')`)
      .limit(limit)
      .orderBy(desc(Reports.createdAt))
      .execute();
